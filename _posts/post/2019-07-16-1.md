...
outline: >
  Apple IDは、Apple社のサービスを利用するのに必要なアカウントであり、ユーザーを一意に特定するためのIDです。
  以前別の記事でも言及しましたが、2019年7月現在では少々状況が変化しています。
  今回は、Apple IDにフォーカスして2019年7月時点の教育・企業の組織における運用方法等を紹介します。
categories:
...
また、本人を特定するための追加情報として、秘密の質問または電話番号が必要です。
...
組織において、個人で使用するApple IDと同様にApple IDを作成すると、いくつかの課題が起こる可能性があります。
...
組織において、多くの方はApp Storeで販売されているアプリケーションを利用するためにApple IDを使用していると思います。
このApp Storeを経由してアプリケーションを使用するには、注意すべき事項があります。
...
ここでポイントとなるのが、**個人で所有または管理していない端末**でのApp Storeのアプリケーション（無料・有料問わず）の使用は**規約違反の可能性がある**ということです。

組織においてはしばしば、組織が貸与するmacOSやiOSの端末が使用されます。
これらの端末上で通常のApple IDを使用してApp Storeを使用する場合、「ライセンスの範囲」と照らし合わせてみると、**規約違反になりうる**と解釈できます。

規約に準拠していない運用は、教育・企業としてあるべき姿ではないため、このような運用は見直す必要があります。

代替手段としては、[Apple製品を会社として導入するための準備をした話]({% post_url post/2017-12-03-1 %})でも言及しました `Volume Purchase Program` を使用すると、Apple社の規約に沿った形でApp Storeで販売されているアプリケーションを使用できます。
