...
<dict>
  ...
  <array>
    ...
    <string>-c</string>
    <string>ps xc | grep "${APPLICATION_NAME}" || rm -rf "\${HOME}/Library/Application Support/com.jamfsoftware.selfservice.mac/CocoaAppCD.storedata" &amp;&amp; sleep "${THROTTLE_INTERVAL}"</string>
  </array>
...
```sh
/bin/sh -c ps xc | grep "設定したアプリケーション名" || rm -rf "${HOME}/Library/Application Support/com.jamfsoftware.selfservice.mac/CocoaAppCD.storedata" && sleep "設定したThrottle Interval"
```
