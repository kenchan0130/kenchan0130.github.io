...
<dict>
  ...
  <array>
    ...
    <string>-c</string>
    <string>rm -rf "$HOME/Library/Application Support/com.jamfsoftware.selfservice.mac/CocoaAppCD.storedata" &amp;&amp; sleep ${THROTTLE_INTERVAL}</string>
  </array>
...
監視に引っかかった場合、`ProgramArguments`で定義されているプログラムが起動します。

```sh
/bin/sh -c rm -rf "$HOME/Library/Application Support/com.jamfsoftware.selfservice.mac/CocoaAppCD.storedata" && sleep ${THROTTLE_INTERVAL}
```
...
最後は、`sleep ${THROTTLE_INTERVAL}`です。
