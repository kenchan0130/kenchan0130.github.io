...
* **「S3バケットの静的Webサイトホスティングを使用」の解決策 - バケットポリシーでRefererの値を参照する方法**
  * `Referer`が分かれば、直接S3バケットの静的Webサイトホスティングにアクセスできる

この中でコスト増加リスク、およびメンテナンスコストが低いと考えられるものを選択したいと考えたため、最終的には **「S3バケットの静的Webサイトホスティングを使用」の解決策 - バケットポリシーでRefererの値を参照する方法** を選択しました。

## 終わりに

どの選択肢も、若干の懸念点はある状態にはなってしまったため、もしより良い方法をご存じの方はご教示ください。

今回挙げた方法以外にも、[AWS Amplify](https://aws.amazon.com/jp/amplify/)を使用する方法があります。
