...
Apple IDは、Apple社のサービスを利用するのに必要なアカウントです。
...
今回は、Apple IDにフォーカスして2019年7月時点の教育・企業における運用方法等を紹介します。
...
Apple IDはApple社がユーザーを一意に特定するためのIDです。
...
## 教育・企業におけるApple IDに関する課題

教育・企業において、個人で使用するApple IDと同様にApple IDを作成すると、いくつかの課題が起こる可能性があります。
...
また、App Storeアプリケーションの規約については注意すべき事項があります。
...
ここでポイントとなるのが、**個人で所有または管理していない端末**でのApp Storeのアプリケーション（無料・有料問わず）の使用は**ライセンス違反**の可能性があるということです。

教育・企業においてはしばしば、組織が貸与するmacOSやiOSの端末が使用されます。
これらの端末上で通常のApple IDを使用してApp Storeを使用する場合、「ライセンスの範囲」と照らし合わせてみると、 **通常のApple IDを使用してStoreのアプリケーションを使用することはライセンス違反になりうる** と解釈できます。

規約に準拠していない運用は、教育・企業としてあるべき姿ではないため、このような運用は見直す必要があります。
...
前置きが長かったですが、教育・企業におけるApple IDに関する課題を解決するためのソリューションが `Managed Apple ID` です。「管理対象Apple ID」とも呼ばれます。
...
`Managed Apple ID` と通常のApple IDには以下のような違いがあります。
...
Managed Apple IDには上記のように一部制限があるものの、通常のApple IDには存在せず教育・企業で有用になりうる機能が存在します。

### Managed Apple IDの作成方法

前提条件として、必要要件を満たしApple社の審査を通過しABM/ACMのグラウドポータルにアクセスできている必要があります。

Managed Apple IDは、ASMまたはABMのポータルから作成したいApple IDのメールアドレス宛に送付するメールのリンクを経由して作成できます。
[Apple ID作成フォーム](https://appleid.apple.com/account)から個人で作成してしまうと、以下のように少々面倒なマイグレーションを行う必要があるため注意が必要です。
...
しかし、教育・企業の現場ではしばしば `A@example.com` をManaged Apple IDにしたいケースがあります。
