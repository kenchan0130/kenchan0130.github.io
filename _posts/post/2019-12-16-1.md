...
この記事は[corp-engr 情シスSlack（コーポレートエンジニア x 情シス） Advent Calendar 2019](https://adventar.org/calendars/3995) 12月16日の記事でもあります。
...
Apple Business Manager（以下、ABM）は企業向け、Apple School Manager（以下、ASM）は（学）などの教育機関向けという違いであり、中身はほぼ同一の機能を有しています。


## ABM/ASMの申請

ABM/ASMを使用するためには以下の手順に沿って申請します。

1. D-U-N-S®番号の発行
2. 組織の登録と審査
3. Managed Apple IDの登録
4. (Optional) Managed Apple IDのFederated Authenticationの設定
...
ABM/ASMにはこの番号が事前に必要となるので、発行する必要があります。
...
1. [D-U-N-S Number look up tool](https://developer.apple.com/enroll/duns-lookup/)を使用して英語表記の法人名でリクエストを投げます。
...
### Managed Apple IDのFederated Authenticationの設定
...
ABMはDevice Enrollment Program（以下、DEP）の設定ができます。
...
ABMはVolume Purchase Program（以下、VPP）の設定できます。

### 場所の設定

ABMには**場所**という概念があります。
VPPはこの**場所**に紐づくという特徴があります。
DEPはABM自体に紐づいていましたが、VPPでは異る点に注意が必要です。

**場所**はデフォルトでABM申請時の住所が存在しており、この**場所**にVPPを紐づけることができます。
...
具体的にはMDMサーバに紐づく場所を以下のように設定すると良いです。
...
ABM/ASMの話とは少し外れますが、Jamf ProなどのMDMサービスを使用する際には、[Apple Push Certificates Portal](https://identity.apple.com/pushcert/)で作成したプッシュ証明書を各種MDMサーバに登録する作業が一年毎に必要です。
