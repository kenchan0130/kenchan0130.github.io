---
title: Circle CI を使用した GitHub Pages のデプロイ
layout: post
outline: >
...
Hexo や Jekyll など静的なサイトジェネレーターを使用して Web ページを生成することが可能となります。
...
この方法は、ビルドをローカル環境で行わずよく、公開前にテストなど様々なビルドが可能となります。
...
VuePress はデフォルトで `.vuepress/dist` にビルドされます。
...
jobs:
  build:
    working_directory: ~/github-pages-vuepress-sample
    docker:
      - image: node:9.11
        environment:
...
2. デプロイ内容物かあるかの確認
3. ビルド結果をコミット
4. GitHub のリモート master ブランチにコミットをプッシュ

を順に行っています。

重要なのはコミットメッセージに `[ci skip]` を含めていることです。

このコミットがマージされた際のビルドでは CI が走りません。
...
Circle CI と連携されていないと GitHub にプッシュしてもビルドが走りません。
...
GitHub Pages で楽にかつ安く静的サイトホスティングしていきましょう！
