...
outline: >
  ...
  単体でサーバーのプロビジョニングや管理なしでスクリプト実行が可能な JavaScript ライクな言語または仕組みです。
  今回は Google Apps Script のモダンな開発方法を検討したため、その記録を公開します。
categories:
  - Development
tags:
  - Google Apps Script
...
Google Apps Script では Web アプリケーションとして公開すると、無料で HTTP GET や HTTP POST のエンドポイントを実現できることがわかりました。

そのため、例えば HTTP POST を使用することで、Slack Bot アプリ用のサーバーとして機能させることができます。
また、 HTTP GET を 使用することで、定期的に更新される HTML や JSON API などを RSS として配信して Slack で連携することができます。
...
そんな中、 2018 年 1 月に満を持して Google 社が Google Apps Script をローカル環境で開発するための CLI となる [clasp](https://github.com/google/clasp) を OSS で [NPM ライブラリ](https://www.npmjs.com/package/@google/clasp)として提供を開始しました。
...
最低限の動作は確認できていますが、絶賛拡張中のため、詳しい内容に関してはまた後日ブログを書く予定です。
...
まだまだ発展途上のため、より便利に開発を行いたい場合は `webpack` と `babel` と引き続き使用しつつける必要がありますが、 `clasp` は開発速度が早いため今後の対応に引き続き目が話せません。

## 宣伝

2018 年 12 月現在、 FOLIO では[絶賛採用強化中](https://www.wantedly.com/companies/folio/projects)です。
ご興味ある方はぜひご連絡ください！
