---
title: 1 つの画面で効率よく iOS アプリ開発のデバックを行う方法
layout: post
outline: >
  iOS アプリケーションを開発しているとシミュレータでデバックすることが多くあります。
  しかし、通常では iOS シミュレータが全画面にできず、画面が大きくない場合、開発しているエディタとスイッチする必要があります。
  今回は、隠れ機能を使用して iOS シミュレータを全画面可能なようにする手順を紹介します。
  また、合わせてその他の隠れ機能も一部紹介します。
categories:
  - Development
tags:
  - AppleInternal
...
iOS アプリケーションを開発しているとシミュレータでデバックすることが多くあります。
しかし、通常では iOS シミュレータが全画面にできず、画面が大きくない場合や複数の画面を使用していない環境の場合、
Xcode などの開発アプリケーションとスイッチする必要があります。

今回は、隠れ機能を使用し、iOS シミュレータを全画面表示を有効にして、
...
`/AppleInternal` ディレクトリは Apple 社の内部用に用意されたフォルダです。
このディレクトリが存在する場合、iOS シミュレータの隠された機能が有効化されます。
...
High Sierra (10.13) から `/AppleInternal` ディレクトリ作成が System Integrity Protecton (SIP) の対象となり、
...
Sierra (10.12) では上記のコマンドを実行してもフォルダを作成することができていました。

### リカバリモードを用いたディレクトリの作成

SIP を無効にすることで、ディレクトリを作成することが可能ですが、セキュリティの観点からあまりオススメできません。

今回は SIP は無効にせずに、リカバリモードを使用すること実現させます。

1. Mac をシャットダウンします。
2. リカバリモードとして Mac を立ち上げます。
  - `⌘ + R` を押しながら Mac の電源を入れます。
  - Apple のロゴが出てきたら `⌘ + R` のキーを離してください。
3. ディスクユーティリティを起動します。
4. macOS がインストールされているディスクを選択して「マウント」ボタンを押します。
5. ディスクユーティリティを終了します。
6. メニューバーの「ユーティリティ > ターミナル」を選択してターミナルを起動します。
7. `mkdir -p /Volumes/Macintosh\ HD/AppleInternal` を実行します。
8. メニューバーの「Apple マーク > 再起動」を選択して再起動を行います。

#### 実行例
私の例ですが上記の作業を全てターミナルで実行した結果を載せておきます。
...
{% lightbox %}
![iOS シミュレータの起動後]({% asset_path launch_ios_simulator.png %})
{% endlightbox %}
...
| iOS シミュレータを選択します。                                   | {% lightbox %}![分割画面における iOS シミュレータの選択]({% asset_path other_select_in_split_view.png %}){% endlightbox %} |

これにより画面が分割され、1 つの画面で開発が可能となりました。

{% lightbox %}
![分割画面での開発]({% asset_path split_view_development.png %})
{% endlightbox %}

私は 11 インチの Macbook Air を使用しており、スイッチイングを行わずに済み、とても重宝しています。
...
|       **UI のコンテンツ枠を表示する機能**        |       {% lightbox %}![UI のコンテンツ枠を表示する機能]({% asset_path internal_function_show_chrome_ui_rects.png %}){% endlightbox %}       |
...
`/AppleInternal` ディレクトリを作成することで、iOS シミュレータの隠し機能を有効にできました。

また、1 つの画面でデバックが実現可能となり、画面サイズに制約がある場合に効率よくデバックできる方法を紹介しました。

合わせて、iOS シミュレータの隠し機能の一部も紹介しました。

冒頭にもお話しましたが、これはディベロッパー向けに公開されている機能ではないため、予期せぬ挙動を起こす可能性があります。
あくまでこの機能に関しては自己責任でお願いします。

それでは素敵な iOS 開発ライフをお送りください！
