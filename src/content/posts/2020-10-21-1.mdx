...
1. Firebaseのプロジェクトと同じ名前のプロジェクトが存在するので、対象のプロジェクトを開きます。
    ...
    ![GCPコンソール上でのプロジェクトの選択](/assets/posts/post/2020-10-21-1/gcp_console_select_project.png)

1. メニューからCloud Storageのページに遷移します。

    ![GCPコンソール上でのCloud Storageページへ移動](/assets/posts/post/2020-10-21-1/gcp_console_storage.png)
...
1. [ライフサイクル]タブから、[ルールを追加]を選択します。
    ...
    ![GCPコンソール上でのCloud Storageのバケットのライフサイクル画面](/assets/posts/post/2020-10-21-1/gcp_console_storage_bucket_lifecycle.png)

1. アクションとして[オブジェクトを削除]を選択します。

    ![GCPコンソール上でのCloud Storageのバケットのライフサイクルアクション設定画面](/assets/posts/post/2020-10-21-1/gcp_console_storage_bucket_lifecycle_action.png)

1. オブジェクト条件として[年齢]を選択し、**3日**とします。

    * ここでは**3日**としていますが、ご自身の都合に合わせて日付を変更してください。

    ![GCPコンソール上でのCloud Storageのバケットのライフサイクル条件設定画面](/assets/posts/post/2020-10-21-1/gcp_console_storage_bucket_lifecycle_object_condition.png)
