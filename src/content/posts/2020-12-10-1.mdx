...
description: AWS Systems ManagerにSession Managerという機能があります。 Session Managerを使用することで、専用の踏み台サーバーの用意や、SSHおよびRDPポートを開けずに、ブラウザやCLIからプライベートネットワークにアクセスできます。 また、セッションログを出力でき、実際に操作した人やその内容の監査が可能です。 今回は、Session Managerを用いて、プライベートネットワークにセキュアにアクセスするための環境を構築する方法を紹介します。
...
![今回構築するインフラ構成図](/assets/posts/post/2020-12-10-1/infra_diagram.png)
...
![AWSのWebコンソール上のマネージドインスタンス一覧](/assets/posts/post/2020-12-10-1/managed_instance_list_view_at_aws_web_console.png)
...
![AWS WebコンソールでのECSのタスク実行](/assets/posts/post/2020-12-10-1/ecs_task_execution_with_aws_web_console.png)
