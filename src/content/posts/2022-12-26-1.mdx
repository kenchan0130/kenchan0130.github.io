...
description: Azure Active Directory（Azure AD）には「エンタープライズアプリケーション」と「アプリ登録」と呼ばれる管理項目があります。 特に、SAMLやOIDCを利用するアプリケーションを登録するなど、運用する際に使用されることが多いでしょう。 今回はこれらをInfrastructure as Code（IaC）のデファクトスタンダードとなりつつあるTerraformで管理する方法を紹介します。
...
![OIDCでログインする際に管理者権限でAPIアクセスの許可を委任してない場合に表示されるダイアログ](/assets/posts/post/2022-12-26-1/oidc_admin_privilege_delegation_dialog.png)
...
![Azure PortalにおけるSAMLの属性とクレームの変更画面](/assets/posts/post/2022-12-26-1/saml_attributes_and_claims_at_azure_portal.png)
