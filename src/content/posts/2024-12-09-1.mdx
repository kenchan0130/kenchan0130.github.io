...
description: 主に管理されているmacOSデバイスに、最新バーションのアプリケーションを配布する方法として、Installomatorというスクリプトがあります。 今回はInstallomatorをより深く理解するために、Installomatorで使用できるパラメータたちを紹介します。
...
以前、[Jamf ProとInstallomatorで常に最新のアプリケーションを配布する](/post/2020-10-12-1)の記事では基本的な使い方を紹介しました。
今回はInstallomatorをより深く理解するために、Installomatorで使用できるパラメータたちを紹介します。

この記事はv10.5時点の情報です。バージョンアップ後の内容とは異なる場合があります。
...
以下の表では、初期状態で選択される値に「デフォルト」と付記しています。
...
|------------------|-----------------------------------------------------------------------------------------------------|
| `0`              | デバッグモードを無効化する。                                                                                      |
| <span class="default-value"><code>1</code><span class="default-label">デフォルト</span></span> | デバッグモードを有効化する。スクリプトを実行したディレクトリにインストーラがダウンロードされる。また、バーションチェックは行われない。                                 |
...
| 値                            | 説明                          |
|------------------------------|-----------------------------|
| `DEBUG` | `DEBUG`が有効な場合のデフォルト。すべてのログを出力する。 |
| <span class="default-value"><code>INFO</code><span class="default-label">デフォルト</span></span> | 通常のログを出力する。                 |
| `WARN`                       | 警告のログのみを出力する。               |
| `ERROR`                      | エラーのログのみを出力する。              |
...
| 値                  | 説明                               |
|--------------------|----------------------------------|
| <span class="default-value"><code>success</code><span class="default-label">デフォルト</span></span> | インストールが成功した場合に通知を行う。             |
| `silent`           | 通知をしない。                          |
| `all`              | ダウンロード中、更新不要、インストール完了、失敗時に通知を行う。 |

通知方法の優先順位は以下のとおりです。

1. `NOTIFY_DIALOG`パラメータが有効で、swiftDialogのバージョン2以上がインストールされている場合は、swiftDialogが使用される。
2. Jamf Proでエンロールされている場合は、`Management Action.app`が使用される。
3. Workspace ONEでエンロールされている場合は、`Hub CLI`が使用される。
4. swiftDialogのバージョン2以上がインストールされている場合は、swiftDialogが使用される。
5. AppleScriptを使用した通知センターへの通知が使用される。
...
| 値                | 説明                    |
|------------------|-----------------------|
| <span class="default-value"><code>0</code><span class="default-label">デフォルト</span></span> | 通知にswiftDialogを使用しない。 |
...
| 値                    | 説明                                                                                                                            |
|----------------------|-------------------------------------------------------------------------------------------------------------------------------|
| <span class="default-value">`appstore`<span class="default-label">デフォルト</span></span> | App Storeのアイコンを使用する。                                                                                                          |
...
| <span class="default-value"><code>tell_user</code><span class="default-label">デフォルト</span></span> | ダイアログを表示し、ユーザーに「Quit and Update」のみを選択させる。                                                                                         |
...
`BLOCKING_PROCESS_ACTION`変数に次の値を指定した場合が対象です。

* `prompt_user`
* `prompt_user_then_kill`
* `prompt_user_loop`

ダイアログのタイムアウト時間を、秒単位で設定します。
...
| 値              | 説明                                                                         |
|----------------|----------------------------------------------------------------------------|
| <span class="default-value"><code>no</code><span class="default-label">デフォルト</span></span> | App Storeからインストールされたアプリケーションが存在する場合、Installomatorを終了する。                    |
...
[Sparkle](https://sparkle-project.org/)など、ユーザー権限で動作する組込みの更新メカニズムからアプリケーションを更新する場合に有用です。

| 値              | 説明                                                                                                               |
|----------------|------------------------------------------------------------------------------------------------------------------|
| <span class="default-value"><code>0</code><span class="default-label">デフォルト</span></span> | dmg、zip、tbz、bz2ファイルから解凍してインストールするアプリケーションの場合、ファイル権限としての所有者を現在のユーザーにする。ただし、ユーザーがログインしていない場合に実行された場合は、所有者はrootになる。 |
...
| 値              | 説明                                                                      |
|----------------|-------------------------------------------------------------------------|
| <span class="default-value">空文字<span class="default-label">デフォルト</span></span> | インストールするアプリケーションのバージョンが現在インストールされているアプリケーションと異なる場合のみ、アプリケーションをインストールする。 |
...
| 値               | 説明                                             |
|-----------------|------------------------------------------------|
| <span class="default-value"><code>yes</code><span class="default-label">デフォルト</span></span> | 更新時にアプリケーションを終了した場合、インストールが終わり次第アプリケーションを起動する。 |
| `no`            | アプリケーションを起動しない。                                |
...
| 値               | 説明                                                      |
|-----------------|---------------------------------------------------------|
| <span class="default-value"><code>yes</code><span class="default-label">デフォルト</span></span> | ディスプレイアサーションの有効に関係なくアプリケーションをインストールする。                  |
...
ラベルに対応するアプリケーション名を、Installomatorスクリプトの末行へ出力するかどうかを制御する設定です。
...
| `appName`                    | `name`変数で定義した値とは異なるアプリケーション名である場合に設定。デフォルトは`$name.app`。                                                                                                            |
...
| `updateTool`                 | アプリケーションの更新時に新しいバージョンをダウンロードしてインストールする代わりに実行するコマンドを指定。アプリケーションがすでにインストールされている、または`type`パラメータが`updateronly`の場合のみ使用される。                                          |
...
| `CLIInstaller`               | `.app`内のインストーラコマンドパス。`.app`内のインストーラコマンドを実行してアプリケーションをインストールする形式をサポート。                                                                                        |
| `CLIArguments`               | `CLIInstaller`パラメータで指定したコマンドの引数の配列。                                                                                                                           |
| `installerTool`              | インストーラの`.app`が、`name`や`appName`パラメータで指定された値とは異なる場合、インストーラのアプリケーション名を指定。                                                                                      |
...
また、このパラメータに関する内容を応用した、「[Installomatorでサポートされていないアプリケーションをインストールする](/post/2024-12-09-2)」という記事も合わせて書きましたので、ぜひご覧ください。
